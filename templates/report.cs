...
<div id="ctxtnav" class="nav">
 ...
 <ul><li class="first"><?cs
   if:chrome.links.up.0.href ?><a href="<?cs
    var:chrome.links.up.0.href ?>">可供使用的報表</a><?cs
   else ?>可供使用的報表<?cs
  /if ?></li><?cs
  if:report.query_href ?><li class="last"><a href="<?cs
   var:report.query_href ?>">自訂查詢</a></li><?cs
  /if ?></ul>
</div>
...
<div id="content" class="report">
 ...
 <?cs if:report.mode == "list" ?>
  <h1>
  <?cs if title == "Create New Report" ?>建立新報表
  <?cs elif title == "Edit Report" ?>編輯報表
  <?cs else ?>
    <?cs var:title ?>
  <?cs /if ?>
  <?cs
   if:report.numrows && report.id != -1 ?><span class="numrows"> (符合數:<?cs
    var:report.numrows ?>)</span><?cs
   /if ?></h1><?cs
 ...
 <?cs elif:report.mode == "edit" ?>
   ...
   <form action="<?cs var:report.href ?>" method="post">
    <div>
     ...
     <div class="field">
      <label for="title">報表標題:</label><br />
      <input type="text" id="title" name="title"
             value="<?cs var:report.title ?>" size="50" /><br />
     </div>
     <div class="field">
      <label for="description">
       描述:</label> (您能使用 <a tabindex="42" href="<?cs
         var:$trac.href.wiki ?>/WikiFormatting">WikiFormatting</a> 來撰寫)
      </label><br />
...
<?cs var:report.description ?></textarea>
 ...
 <div id="help">
  <strong>注意:</strong> 請參考 <a href="<?cs
    var:trac.href.wiki ?>/TracReports">TracReports</a> 以取得使用與建立報表的說明.
 </div>
