...
<div id="ctxtnav" class="nav">
 ...
 <ul><?cs
  if:wiki.action == "diff" ?>
   ...
   <li><a href="<?cs var:trac.href.wiki ?>">起始頁面</a></li>
   <li><a href="<?cs var:trac.href.wiki ?>/TitleIndex">依標題檢索</a></li>
   <li><a href="<?cs var:trac.href.wiki ?>/RecentChanges">依日期檢索</a></li>
   <li class="last"><a href="<?cs var:wiki.last_change_href ?>">最後變更</a></li><?cs
  /if ?>
 </ul>
 <hr />
</div>
...
<div id="content" class="wiki">
 ...
 <?cs else ?>
  <?cs if wiki.action == "edit" || wiki.action == "preview" || wiki.action == "collision" ?>
   <h1>編輯 "<?cs var:wiki.page_name ?>"</h1><?cs
   ...
   <form id="edit" action="<?cs var:wiki.current_href ?>" method="post">
    <fieldset class="iefix">
     ...
     <div id="rows">
      <label for="editrows">調整編輯區域高度:</label>
...
<?cs var:wiki.page_source ?></textarea></p>
    ...
    <div id="help">
     <b>注意:</b> 參考 <a href="<?cs var:$trac.href.wiki
?>/WikiFormatting">WikiFormatting</a> 與 <a href="<?cs var:$trac.href.wiki
?>/TracWiki">TracWiki</a> 以取得編輯wiki內容之說明.
    </div>
    ...
    <div class="buttons"><?cs
     if wiki.action == "collision" ?>
      <input type="submit" name="preview" value="預　　覽" disabled="disabled" />&nbsp;
      <input type="submit" name="save" id="btnSave" value="送交變動" title="按Ctrl+Enter立即儲存" disabled="disabled" />&nbsp;
     <?cs else ?>
      <input type="submit" name="preview" value="預　　覽" accesskey="r" />&nbsp;
      <input type="submit" name="save" id="btnSave" value="送交變動" title="按Ctrl+Enter立即儲存" />&nbsp;
     <?cs /if ?>
     <input type="submit" name="cancel" id="btnCancel" value="取　消" />
    </div>
   ...
   <?cs if:len(wiki.attachments) ?>
    ...
    <ul class="tkt-chg-list"><?cs
     each:attachment = wiki.attachments ?><li class="tkt-chg-change"><a href="<?cs
      ...
      /if ?> 加入者: <?cs var:attachment.author ?> 日期: <?cs
   ...
   <div class="buttons"><?cs
    if:trac.acl.WIKI_MODIFY ?>
     <form method="get" action="<?cs var:wiki.current_href ?>"><div>
      <input type="hidden" name="action" value="edit" />
      <input id="btnEdit" type="submit" value="<?cs if:wiki.exists ?>編輯<?cs
        else ?>建立<?cs /if ?>此頁" accesskey="a" title="Alt+A或雙擊立即進入編輯"/>
     </div></form><?cs
     if:wiki.exists ?>
      <form method="get" action="<?cs var:wiki.attach_href ?>"><div>
       <input type="hidden" name="action" value="new" />
       <input type="submit" value="附件上傳" />
      </div></form><?cs
     /if ?><?cs
    /if ?><?cs
...
<script>
  var _isEditing = <?cs if wiki.action == "edit" ?>true;<?cs else ?>false;<?cs /if ?>
  ...
  document.onkeypress = function(event) {
    if (_isEditing) {
      ...
      var _iKeyCode = event.keyCode;
      if (_iKeyCode == 10) { // IE: Ctrl+Enter
        document.getElementById("btnSave").click();
      } else if (_iKeyCode == 13 && event.ctrlKey == 1) {
        document.getElementById("btnSave").click();
      }
    }
  }

  var element = document.getElementById("btnEdit");
  if (element != null) {
