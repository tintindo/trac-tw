...
<div id="content" class="milestone">
 ...
 <h1>編輯里程碑 <?cs var:milestone.name ?></h1>
 <?cs elif:milestone.mode == "delete" ?>
 <h1>刪除里程碑 <?cs var:milestone.name ?></h1>
 <?cs else ?>
  ...
  <form id="edit" action="<?cs var:milestone.href ?>" method="post">
   ...
   <fieldset>
    ...
    <label>到期日:<br />
     <input type="text" id="duedate" name="duedate" size="<?cs
       ...
       var:milestone.due_date ?>" title="格式: <?cs var:milestone.date_hint ?>" />
     <em>格式: <?cs var:milestone.date_hint ?></em>
    </label>
    <div class="field">
     <label>
     ...
     <label>
      ...
      <em>格式: <?cs var:milestone.datetime_hint ?></em>
     </label><?cs
     if:len(milestones) ?>
     <br/>
   ...
   <div class="field">
    <fieldset class="iefix">
     <label for="description">描述 (您能使用 <a tabindex="42" href="<?cs
       var:trac.href.wiki ?>/WikiFormatting">WikiFormatting</a> 來撰寫):</label>
...
<?cs var:milestone.description_source ?></textarea></p>
   ...
   <div class="buttons">
    <?cs if:milestone.mode == "new"
     ...
     ?><input type="submit" value="送交變更" /><?cs
    /if ?>
 ...
 <?cs elif:milestone.mode == "delete" ?>
  <form action="<?cs var:milestone.href ?>" method="post">
   ...
   <p><strong>確定要刪除此里程碑嗎？</strong></p>
   <input type="checkbox" id="retarget" name="retarget" checked="checked"
       onclick="enableControl('target', this.checked)"/>
   <label for="target">重新設定已連結的任務單</label>
  ...
  <div class="info">
   <p class="date"><?cs
    if:milestone.completed_date ?>
     <?cs var:milestone.completed_delta ?> 前已完成 (<?cs var:milestone.completed_date ?>)<?cs
    elif:milestone.due_date ?><?cs
     if:milestone.late ?>
      <strong>逾期 <?cs var:milestone.due_delta ?></strong><?cs
     else ?>
      <?cs var:milestone.due_delta ?> 到期<?cs
